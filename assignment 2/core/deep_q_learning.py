...
# Process state -- Need to cast it to float32
...
# add update operator for target network
    ...
    def initialize(self):
        ...
        # from_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "q")
        # initializer = tf.contrib.layers.variance_scaling_initializer()

        init = tf.global_variables_initializer()
        self.sess.run(init)
        # self.sess.run(tf.initializers.he_normal(from_vars))
...
#         Performs an update of parameters by sampling from replay_buffer
    ...
    def update_step(self, t, replay_buffer, lr):
        ...
        loss_eval, grad_norm_eval, summary, _ = self.sess.run([self.loss, self.grad_norm, self.merged, self.train_op], feed_dict=fd)
