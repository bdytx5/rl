...
class Linear(DQN):
    ...
    """
    # create placeholders for all the necessary vars eg - s, a, r, --> void 
    def add_placeholders_op(self):
        ...
        # self.config.State_history = 4 - eg 4 frames 
    ...
    # implement fully connected layer, -> layers.fully_connected
    ...
    # get the q and targetq vars from their scope, 
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        #   Returns: A `Tensor` that will hold the new value of 'ref' after
        assgin_op = [tf.assign(target_q_scope_variable[i], q_scope_variable[i]) for i in range(len(q_scope_variable))]
       
        # * to unpack the list 
        #   """Create an op that groups multiple operations. no return val 
        self.update_target_op = tf.group(*assgin_op) # idk some sort of operator 
...
#    Sets the loss of a batch, self.loss is a scalar
    def add_loss_op(self, q, target_q):
        ...
        HINT: 
            ...
            - You may find the following functions useful
                ...
                - tf.reduce_max
                - tf.reduce_sum 
                - tf.one_hot -Computes the sum of elements across dimensions of a tensor.
        ...
        # def reduce_sum(input_tensor, axis=None,
        # q: (tf tensor) shape = (batch_size, num_actions)
        # actions is a 1D one-hot probability vector representing action prob.
        actions = tf.one_hot(self.a, num_actions)
        print("----")
        tf.print(actions, output_stream=sys.stdout)
        q = tf.reduce_sum(tf.multiply(q, actions), axis=1) # axis 1 means sum in the y direction 
    ...
    def add_optimizer_op(self, scope):
        ...
        var_in_scope = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope) # get vars with this scope 
        gradients, variables = zip(*optimizer.compute_gradients(self.loss, var_list=var_in_scope)) # zip em?

        gradients = [tf.clip_by_norm(gradient, self.config.clip_val) for gradient in gradients] # clim em?
...
if __name__ == '__main__':
    ...
    exp_schedule = LinearExploration(env, config.eps_begin, config.eps_end, config.eps_nsteps, False)
    # learning rate schedule
    lr_schedule  = LinearSchedule(config.lr_begin, config.lr_end, config.lr_nsteps, True)
    # train model
    model = Linear(env, config)
    model.run(exp_schedule, lr_schedule) # see QN class 
