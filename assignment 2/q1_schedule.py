...
class LinearSchedule(object):
    def __init__(self, eps_begin, eps_end, nsteps,islr):
        """
        Args:
    ...
    def update(self, t):
        ...
        if t < (0.9*self.nsteps*2) and self.islr:
            # self.lrate = self.eps_begin - (float(t)/float(self.nsteps*10))
            self.epsilon = self.eps_begin - (self.eps_begin * float(t)/float(self.nsteps*2))
        elif self.islr:
            self.epsilon = self.eps_end


        if t < (0.9*self.nsteps*2) and not self.islr:
            self.epsilon = self.eps_begin - (self.eps_begin * float(t)/float(self.nsteps*2))
        elif not self.islr:
            self.epsilon = self.eps_end
...
class LinearExploration(LinearSchedule):
    def __init__(self, env, eps_begin, eps_end, nsteps,islr):
        """
        Args:
            env: gym environment
            eps_begin: float
        ...
        """
        self.env = env
        super(LinearExploration, self).__init__(eps_begin, eps_end, nsteps,islr)
